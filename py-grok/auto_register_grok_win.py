...
# Bật hỗ trợ màu ANSI trên Windows
if os.name == 'nt':
    os.system('color')
...
def setup_driver(index=1, keep_open=False):
    ...
    # Ép buộc kích thước và vị trí qua lệnh WebDriver (chắc ăn 100% trên MacOS và Windows)
    try:
...
def register_one_account(index, keep_open=False):
    ...
    try:
        ...
        log(f"Đã mở {GROK_URL}, đợi trang tải và xử lý Turnstile...", "INFO")
        time.sleep(5)
