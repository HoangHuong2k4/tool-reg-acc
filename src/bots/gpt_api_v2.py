...
# Thêm src/ và gpt_engine của V2 vào sys.path để config và core có thể được import đúng cách
_V2_SRC_DIR = os.path.abspath(os.path.join(_BOT_DIR, "..", "..", "capcut-regaccc-v2", "src"))
_GPT_ENGINE_DIR = os.path.join(_V2_SRC_DIR, "gpt_engine")
if _V2_SRC_DIR not in sys.path:
    sys.path.insert(0, _V2_SRC_DIR)
                    ...
                    if messages:
                        for msg in messages:
                            ...
                            if otp:
                                ...
                                if after_ts:
                                    try:
                                        from gpt_engine.core.outlook_client import _parse_email_ts
                                        msg_ts = _parse_email_ts(msg)
                                ...
                                if (has_timestamp and is_new_email) or (not has_timestamp and otp not in seen_otps):
                                    seen_otps.add(otp)
...
def register_one_account(thread_id):
    ...
    lưu kết quả và đánh dấu email đã dùng. Tự động xoay proxy và thử lại nếu lỗi mạng/IP bẩn.
    """
    ...
    while attempt < max_attempts:
        attempt += 1
        proxy_info = get_rotated_proxy()
        proxy_url = _format_proxy(proxy_info)
        
        if attempt == 1:
            log("[Thread-{}] Bắt đầu đăng ký GPT: {}".format(thread_id, email), "INFO")
        else:
            log("[Thread-{}] Thử lại GPT: {} (Lần {}/{})".format(thread_id, email, attempt, max_attempts), "INFO")
        ...
        try:
            ...
            enable_2fa_val = globals().get("ENABLE_2FA", True)
            gem._twofa_cfg.ENABLE_2FA = enable_2fa_val
            _twofa_cfg.ENABLE_2FA = enable_2fa_val
            ...
            if result and result.get("success"):
                ...
                log("[Thread-{}] ✅ Thành công: {} | 2FA: {} | PTTT: {}".format(
                ...
                is_proxy_err = any(x in err_lower for x in ("403", "429", "timeout", "connect", "proxy", "circuit", "mở rộng", "mạng"))

                if is_proxy_err and attempt < max_attempts:
                    log("[Thread-{}] ⚠️ Phát hiện lỗi proxy/mạng ({}). Tiến hành xoay proxy và thử lại sau 10s...".format(thread_id, err), "WARN")
                    try:
                        import requests
                        requests.post("http://127.0.0.1:5050/api/proxy/rotate", timeout=20)
                    except Exception as rexc:
                        log("[Thread-{}] Lỗi khi gọi xoay proxy: {}".format(thread_id, rexc), "WARN")
                    time.sleep(10)
                    continue
                else:
            ...
            if is_proxy_err and attempt < max_attempts:
                log("[Thread-{}] ⚠️ Lỗi exception proxy ({}). Tiến hành xoay proxy và thử lại sau 10s...".format(thread_id, err_msg), "WARN")
                try:
                    import requests
                    requests.post("http://127.0.0.1:5050/api/proxy/rotate", timeout=20)
                except Exception as rexc:
