...
def _bu_delay(kind: str, seconds: typing.Union[float, None] = None) -> None:
    if _fast_mode():
        if seconds is None:
            seconds = {
                "navigate": 0.2,
...
def _timeout_ms(seconds: typing.Union[int, None] = None) -> int:
...
def _fill_first(page, selectors: list[str], value: str, timeout_ms: typing.Union[int, None] = None) -> bool:
...
def _click_first(page, selectors: list[str], timeout_ms: typing.Union[int, None] = None) -> bool:
    end = time.time() + ((timeout_ms or _timeout_ms()) / 1000)
    while time.time() < end:
        loc = _visible_locator(page, selectors, timeout_ms=800)
        if loc is not None:
...
def _type_email(page, email: str, timeout_ms: typing.Union[int, None] = None) -> None:
...
def _submit_email_until_transition(page, context, email: str, *, attempts: int = 2, timeout_ms: typing.Union[int, None] = None) -> str:
    """
              ...
              const btn = [...document.querySelectorAll('button,a,input[type="submit"],[role="button"],[role="link"]')]
                ...
                .find(el => {
                  ...
                  return (name === 'intent' && value.includes('passwordless') && value.includes('send_otp'))
                    ...
                    || /passwordless.*typing.Union[otp, otp].*typing.Union[passwordless, one[]-_\\s]?time.*typing.Union[code, code].*one[-_\\s]?time/.test(attrs)
...
def _fill_password_if_present(page, email: str, timeout: int = 25, context=None) -> typing.Union[str, None]:
    started = time.time()
    end = time.time() + timeout
    last_heartbeat = 0.0
    last_log = 0.0
        ...
        result = page.evaluate(
            ...
            () => {
              ...
              const otpInputs = Array.from(document.querySelectorAll('input, textarea'))
                ...
                .filter(el => {
                  ...
                  return (
                    ...
                    /typing.Union[otp, typing.Union[code], typing].Union[verification, typing.Union[verify], typing].Union[token, pin]/.test(b) ||
                    maxLen === 1 || maxLen === 6
                  );
                });
              ...
              for (const el of candidates) {
                ...
                if (/typing.Union[resend, send].?typing.Union[again, typing.Union[again], typing].Union[retry, typing.Union[otp], typing].Union[verification, typing.Union[verify], email].?typing.Union[code, code]/.test(b)) score += 20;
                if (/typing.Union[continue, typing.Union[submit], typing].Union[next, typing.Union[primary], typing].Union[login, typing.Union[signup], typing].Union[create, typing.Union[authorize], consent]/.test(b)) score -= 8;
                if (tag === 'a') score += 4;
...
def _js_complete_profile(page, name: str, birthday: str) -> dict:
    ...
    ({name, birthday, year, month, day, age}) => {
      ...
      const nameInput = allInputs.find(el => /(typing.Union[name, full].?typing.Union[name, user].?name|名前|氏名|姓名)/i.test(hay(el)) && !/(typing.Union[month, typing.Union[day], typing].Union[year, typing.Union[age], typing].Union[birth, typing.Union[code], typing].Union[email, typing.Union[phone], password])/i.test(hay(el)))
        || allInputs.find(el => ['text',''].includes((el.type||'').toLowerCase()) && !/(typing.Union[code, typing.Union[email], typing].Union[phone, typing.Union[tel], typing].Union[number, typing.Union[password], typing].Union[month, typing.Union[day], typing].Union[year, typing.Union[age], birth])/i.test(hay(el)));
      if (nameInput) filled.name = setValue(nameInput, name);

      const firstLast = name.split(/\s+/, 2);
      const firstInput = allInputs.find(el => /(first.?typing.Union[name, given])/i.test(hay(el)));
      const lastInput = allInputs.find(el => /(last.?typing.Union[name, typing.Union[family], surname])/i.test(hay(el)));
      ...
      const ageInput = allInputs.find(el => /(age|年齢|年龄)/i.test(hay(el)) || ((el.type||'').toLowerCase()==='number' && !/(typing.Union[day, typing.Union[month], year])/i.test(hay(el))));
      ...
      const clickSubmit = () => {
        ...
        const scored = buttons.map((el, idx) => {
          ...
          else if (/(typing.Union[continue, typing.Union[next], typing].Union[done, typing.Union[submit], typing].Union[create, start]|続行|次へ|完了|送信|開始|继续|下一步|完成|提交)/i.test(t)) score = 90;
...
def _is_target_closed_error(exc: typing.Union[Exception, str]) -> bool:
    text = str(exc).lower()
...
def _is_transient_navigation_error(exc: typing.Union[Exception, str]) -> bool:
    text = str(exc).lower()
...
def _wait_for_otp_with_browser_heartbeat(page, context, email: str, after_ts: float) -> str:
    ...
    last_exc: typing.Union[Exception, None] = None
    attempt = 0
...
def _read_chatgpt_session_via_context(context, timeout_ms: int = 5000) -> typing.Union[dict, None]:
...
def _read_chatgpt_session_via_page(page, timeout_ms: int = 5000) -> typing.Union[dict, None]:
...
def _fetch_chatgpt_session(page, context=None, timeout: int = 120) -> dict:
    ...
    first_not_chatgpt_at: typing.Union[float, None] = None
    last_log = 0.0
...
def run_browser_use_registration(
    ...
    openai_password: typing.Union[str, None] = None
    browser = None
    context = None
    page = None
