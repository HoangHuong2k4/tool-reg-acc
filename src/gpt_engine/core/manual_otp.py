...
def mark_waiting(email: str, job_id: typing.Union[int, None] = None) -> None:
    key = _norm(email)
    with _lock:
        _waiting[key] = {
            "email": email,
...
def pop_manual_otp(email: str) -> typing.Union[str, None]:
    key = _norm(email)
    with _lock:
...
def wait_for_manual_otp(email: str, *, timeout: int = 180, job_id: typing.Union[int, None] = None) -> str:
