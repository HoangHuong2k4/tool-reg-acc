...
_MIN_MAX_WORKERS = 1
_MAX_MAX_WORKERS = 16
_executor: typing.Union[ThreadPoolExecutor, None] = None
...
def is_stop_requested(job_id: typing.Union[int, None] = None) -> bool:
    if job_id is None:
...
def _release_unconsumed_job_email(email: typing.Union[str, None], reason: str) -> None:
...
def _disable_job_email(email: typing.Union[str, None], reason: str) -> bool:
    """把本次任务邮箱停用，避免后续再次领取。"""
    if not email:
        return False
    try:
...
def _normalize_workers(max_workers: typing.Union[int, None]) -> int:
...
def get_executor(max_workers: typing.Union[int, None] = None) -> ThreadPoolExecutor:
    """返回注册线程池。
...
class _JobLogContext:
    ...
    def __init__(self, log_path: str):
        ...
        self.handler: logging.typing.Union[FileHandler, None] = None
...
def submit_registration(count: int = 1, email_source: typing.Union[str, None] = None, workers: typing.Union[int, None] = None) -> list[dict]:
    """
    创建 N 个注册任务并提交到线程池。
...
def _account_for_job(job: dict) -> typing.Union[dict, None]:
...
def retry_job(job_id: int, workers: typing.Union[int, None] = None) -> dict:
