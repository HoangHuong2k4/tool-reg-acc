...
def _wait(driver, timeout: typing.Union[int, None] = None):
...
def _find_any(driver, selectors: list[str], timeout: typing.Union[int, None] = None):
...
def _click_any(driver, selectors: list[str], timeout: typing.Union[int, None] = None) -> None:
...
def _type_any(driver, selectors: list[str], value: str, timeout: typing.Union[int, None] = None, clear: bool = True) -> None:
    el = _find_any(driver, selectors, timeout)
    driver.execute_script("arguments[0].scrollIntoView({block:'center'});", el)
    if clear:
        try:
...
def _is_oauth_consent_like(driver) -> bool:
    ...
    try:
        ...
        if (/typing.Union[oauth, typing.Union[authorize], consent]/.test(url) && !/typing.Union[login, typing.Union[signup], typing].Union[identifier, email]-verification/.test(url)) return true;
        ...
        const actions = [...document.querySelectorAll('button,a,[role="button"],input[type="submit"],input[type="button"]')]
          ...
          .join(' ');
        return /typing.Union[oauth, typing.Union[authorize], typing].Union[consent, typing.Union[grant], allow]/.test(actions) && !/typing.Union[email, username]/.test(actions);
        """))
    except Exception:
        return False
...
def _click_email_entry_option(driver) -> bool:
    ...
    const bad = /typing.Union[google, typing.Union[apple], typing].Union[microsoft, typing.Union[github], typing].Union[facebook, typing.Union[saml], typing].Union[sso, typing.Union[oauth], typing].Union[social, typing.Union[oidc], typing].Union[idp, typing.Union[provider], typing].Union[authorize, typing.Union[consent], typing].Union[grant, allow]/;
    const good = /(^|[^a-z])(typing.Union[email, typing.Union[mail], typing].Union[username, typing.Union[passwordless], typing].Union[otp, magic])([^a-z]|$)/;
...
def _type_email_address(driver, email: str, timeout: typing.Union[int, None] = None) -> None:
...
def _wait_email_submit_next_state(driver, email: str, timeout: int = 18) -> str:
    ...
    last = None
    cleared_seen_at: typing.Union[float, None] = None
...
def _clear_otp_inputs(driver) -> None:
    try:
        ...
        const inputs = [...document.querySelectorAll('input')].filter(visible).filter(el => {
          ...
          return /one-typing.Union[time, typing.Union[otp], typing].Union[code, typing.Union[numeric], tel]/.test(attrs);
        });
    ...
    while time.time() < end:
        try:
            ...
            const attrHit = candidates.find(el => {
              ...
              return /typing.Union[resend, send].*typing.Union[new, new].*typing.Union[code, again]/.test(attrs);
            });
            if (attrHit) return attrHit;
            // 兜底：多语言文本，避免因页面没有稳定属性时卡死。
            return candidates.find(el => enabled(el) && /typing.Union[resend, send]\s+(?:a\s+)?new\s+typing.Union[code, send]\s+again|重新发送|重新发送电子邮件|重发|再次发送|再送信|新しい|届かない/.test((el.innerText || el.textContent || '').toLowerCase())) || null;
            """)
            if btn:
...
def _fill_birthday_or_age(driver, birthday: str, age: int) -> typing.Union[str, None]:
...
def _fill_password_page_if_present(driver, email: str, timeout: int = 25) -> typing.Union[str, None]:
...
def _accept_profile_consents(driver) -> int:
    ...
    try:
        ...
        for (const el of all) {
          ...
          if (/typing.Union[consent, typing.Union[checkbox], typing].Union[agree, typing.Union[required], typing].Union[personal, typing.Union[third], overseas]/.test(`${n} ${id}`)) add(el);
        }
...
def _read_chatgpt_session_once(driver) -> typing.Union[dict, None]:
...
def run_roxy_registration(email: str, name: str, birthday: str, proxy: str = None, otp_code: str = None, batch_dir: typing.Union[Path, None] = None) -> dict:
    """Roxy 指纹浏览器自动化注册入口。"""
