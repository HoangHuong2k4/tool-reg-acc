...
def _wait_for_callback(context, page, timeout: typing.Union[int, None] = None) -> str:
...
def _wait_for_fresh_email_otp(otp_provider, email: str, after_ts: float, used_codes: typing.Union[set[str], None] = None, timeout: int = 90) -> str:
...
def _js_fill_email_fallback(page, email: str) -> bool:
    ...
    (email) => {
      ...
      const score = (el) => {
        ...
        if (/(typing.Union[email, typing.Union[mail], typing].Union[username, typing.Union[loginfmt], identifier]|メール|邮箱|電子郵件)/i.test(hay)) return 100;
...
def _click_email_entry_fast(page) -> bool:
    ...
    () => {
      ...
      const score = (el) => {
        ...
        if (/(typing.Union[continue, typing.Union[sign], log]).{0,20}(typing.Union[email, mail])|typing.Union[email, mail]|メール|邮箱|電子郵件/i.test(hay)) return 100;
        return 0;
      };
...
def _submit_visible_form_or_enter(page) -> bool:
    ...
    () => {
      ...
      const score = (el) => {
        ...
        if (/(typing.Union[continue, typing.Union[next], typing].Union[submit, sign] typing.Union[in, log] in|続行|次へ|送信|继续|下一步|登录|登入)/i.test(text)) return 100;
...
def _wait_after_email_submit(page, timeout: int = 45, dead_tracker: typing.Union[dict, None] = None) -> str:
    end = time.time() + timeout
    while time.time() < end:
...
def _fill_email_and_otp(page, email: str, otp_provider, auth_url: str, dead_tracker: typing.Union[dict, None] = None) -> None:
    ...
    try:
        page.evaluate(
            """() => {
              ...
              const sms = radios.find(el => /^(typing.Union[sms, typing.Union[text], typing].Union[text_message, text]-message)$/i.test(el.value || ''));
...
def _force_set_phone_value(page, phone_e164: str) -> bool:
    ...
    (phone) => {
      ...
      const score = (el) => {
        const hay = [el.type, el.name, el.id, el.autocomplete, el.placeholder, el.getAttribute('aria-label')].join(' ').toLowerCase();
        if (/(typing.Union[phone, typing.Union[tel], typing].Union[mobile, sms]|手机号|手机|電話|携帯)/i.test(hay)) return 100;
...
def _click_phone_continue(page) -> bool:
    ...
    () => {
      ...
      const score = (el) => {
        ...
        if (/(typing.Union[continue, typing.Union[send], typing].Union[next, typing.Union[verify], submit]|続行|送信|次へ|確認|继续|发送|下一步|验证)/i.test(text)) return 100;
        if ((el.type || '').toLowerCase() === 'submit') return 90;
        return 0;
      };
      const target = buttons.map(el => [score(el), el]).filter(x => x[0] > 0).sort((a,b)=>b[0]-a[0])[0]?.[1];
...
def _clear_phone_inputs(page) -> None:
    ...
    () => {
      ...
      for (const el of inputs) {
        const hay = [el.type, el.name, el.id, el.autocomplete, el.placeholder, el.getAttribute('aria-label')].join(' ').toLowerCase();
        if (/(typing.Union[phone, typing.Union[tel], typing].Union[mobile, sms]|手机号|手机|電話|携帯)/i.test(hay) || (el.type || '').toLowerCase() === 'tel') {
...
def _run_browser_use_codex_oauth_once(email: str, otp_provider=None, proxy: typing.Union[str, None] = None, force: bool = False, cloud_provider: str = "browser_use") -> dict:
...
def _run_browser_use_codex_oauth_impl(email: str, otp_provider=None, proxy: typing.Union[str, None] = None, force: bool = False, cloud_provider: str = "browser_use") -> dict:
...
def run_browser_use_codex_oauth(email: str, otp_provider=None, proxy: typing.Union[str, None] = None, force: bool = False, cloud_provider: str = "browser_use") -> dict:
