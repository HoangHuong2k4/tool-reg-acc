...
def run_registration(
    email: str,
    name: str,
    birthday: typing.Union[str, None] = None,
    proxy: str = None,
    otp_code: str = None,
    batch_dir=None,
    ...
    try:
        ...
        # ─── MỚI: Check cổng thanh toán MoMo ────────────────────────
        from gpt_engine.core.momo_checker import check_momo_payment
        has_momo = False
        if check_momo:
            try:
                has_momo = check_momo_payment(session, access_token)
            except Exception as e:
                logger.error(f"[CheckMoMo] Lỗi không mong muốn: {e}")
        ...
        return {"success": task_success, "email": email, "account_id": account_id,
                ...
                "error": task_error, "has_momo": has_momo}
