...
ENV HOME=/home/appuser \
    ...
    PLAYWRIGHT_BROWSERS_PATH=/home/appuser/.cache/ms-playwright \
    RUNTIME_DIR=/app/capcut-regaccc/data
RUN useradd -m -u 10001 appuser
...
RUN apt-get update && apt-get install -y --no-install-recommends \
      xvfb xauth curl tini nodejs chromium chromium-driver \
    && rm -rf /var/lib/apt/lists/*
...
COPY --chown=appuser:appuser . /app/capcut-regaccc/
...
RUN mkdir -p /app/capcut-regaccc/data \
    && chown appuser:appuser /app/capcut-regaccc/data

WORKDIR /app/capcut-regaccc
USER appuser
...
CMD ["xvfb-run", "-a", "python", "src/web/app.py"]
